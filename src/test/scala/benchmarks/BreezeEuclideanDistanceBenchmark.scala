...
object BreezeEuclideanDistanceBenchmark extends DistanceBenchmark {
  val vectors = for {
    n <- ns
  } yield {
  ...
  performance of "Breeze" in {
    measure method "euclideanDistance" in {
      using(vectors) in { case (v1, v2s) =>
        val distanceFunc: (DenseVector[Double]) => Double = { x2 => breeze.linalg.functions.euclideanDistance(v1, x2) }
